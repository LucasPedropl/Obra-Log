...
**Propósito:** único documento vivo com tudo que **ainda não foi implementado**.
Itens já concluídos (maratona P0–P2, hardening de segurança/LGPD de 24/06, CI
básico, forms de obra com RHF+Zod exceto um, `SecurityTab`, migrações RLS
versionadas, etc.) foram removidos deste arquivo.
...
O ObraLog é um **sistema operacional de canteiro com cadastros mestres** —
almoxarifado, ferramentas, EPIs, equipamentos alugados, movimentações e visão
geral por obra. Os bloqueadores críticos de segurança (RLS por obra,
`globalUsers.ts`, IDOR em inventário, headers HTTP, storage privado, Zod nas
actions principais) foram endereçados em 24/06/2026.
...
**Critério de done:** zero `<select>` nativo para dados dinâmicos → usar
`SearchableSelect`.  
**Esforço:** Baixo (2–4h).
...
| `PermissionsContext.tsx`    | `useEffect([pathname])` refaz 3–4 queries Supabase a **cada navegação**                              |
| Hooks de leitura no browser | `useMovimentacoes`, `useToolHistory`, `useEPIHistory` etc. puxam dados via egress Supabase no client |
| `dashboardActions.ts`       | `construction_sites` consultada **duas vezes** na mesma action                                       |
...
### Escalabilidade e custo (gargalos para muitos usuários)

Problemas que amplificam consumo de **egress/requests do Supabase** e custo de
funções na Vercel — críticos antes de escalar (ex.: ~1000 usuários).

#### E1 — `PermissionsContext` recarrega a cada navegação (prioridade alta)

| Campo    | Detalhe                                                                                                                                                |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Problema | `useEffect` depende de `pathname` → toda troca de rota dispara `getSession` + `profiles` + `company_users` + `access_profiles` (3–4 queries) no client |
| Arquivo  | `context/PermissionsContext.tsx` (~L78–L182)                                                                                                           |
| Impacto  | Multiplica egress e nº de requests por usuário; pior ofensor de custo em escala                                                                        |
| Correção | Carregar permissões **uma vez** (layout/server) e cachear; remover `pathname` da dependência                                                           |
| Esforço  | Baixo-médio (4–8h)                                                                                                                                     |

#### E2 — Leituras pesadas direto no browser

| Campo    | Detalhe                                                                                                          |
| -------- | ---------------------------------------------------------------------------------------------------------------- |
| Problema | Histórico/listagens grandes lidas via `createClient()` no client, jogando todo o tráfego pelo egress do Supabase |
| Arquivos | `useMovimentacoes`, `useToolHistory`, `useEPIHistory`, `useActiveLoans`, `useRentedHistory`                      |
| Correção | Migrar leituras volumosas para Server Components/Server Actions com cache (`revalidate`) e paginação             |
| Esforço  | Médio (2–4 dias)                                                                                                 |

#### E3 — Query redundante no dashboard

| Campo    | Detalhe                                                                                                                      |
| -------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Problema | `getDashboardKpisAction` busca `construction_sites` no `Promise.all` (count) e **novamente** logo abaixo para obter os `ids` |
| Arquivo  | `app/actions/dashboardActions.ts` (~L24–L45)                                                                                 |
| Correção | Uma única query trazendo `id` e derivar o count em memória                                                                   |
| Esforço  | Baixo (1–2h)                                                                                                                 |

#### E4 — Limites de plano gratuito (referência)

| Plataforma          | Limite free (~2026)              | Risco em escala                                 |
| ------------------- | -------------------------------- | ----------------------------------------------- |
| Vercel Hobby        | **Uso comercial proibido (ToS)** | Bloqueador jurídico p/ produto pago → Pro       |
| Supabase — DB       | 500 MB                           | Quebra primeiro em ERP com inventário/histórico |
| Supabase — Egress   | 5 GB/mês                         | Crítico devido a E1/E2                          |
| Supabase — Storage  | 1 GB                             | Fotos/documentos estouram                       |
| Supabase — Auth MAU | 50.000                           | Folgado p/ 1000 usuários                        |

**Conclusão:** produção com volume real exige Vercel Pro + Supabase Pro **e** a
correção de E1–E3 para conter egress/custo.
...
> Bloqueadores críticos (SEC-01 a SEC-14, RLS `get_my_sites`, `globalUsers`
> cross-tenant, IDOR inventário, storage público, headers HTTP, cookies
> httpOnly) foram **implementados** em 24/06/2026 via migrations e refactor de
> actions.
...
> Melhorias técnicas já feitas: páginas `/privacidade` e `/termos`, mascaramento
> CPF na UI, `anonymizeCollaboratorAction`, storage privado com signed URLs,
> `exportUtils` anti-injection, `safeLog`, aviso no login.
...
**Medidas de segurança (Art. 32):** RLS multi-tenant, cookies httpOnly, storage
privado, mascaramento CPF na UI.

**Canal do titular:** `privacidade@[dominio-empresa]` — prazo 15 dias (Art. 18,
§3º).
...
**Passos:** contenção → diagnóstico (logs Supabase, RLS) → notificação
ANPD/titulares se risco → remediação → lições aprendidas.
...
3. Informar titulares em `/privacidade` sobre processamento e transferência
   internacional
...
**Módulos inexistentes hoje:** Planejamento · Orçamento · Cronograma · Diário de
obra · Medições · Compras · Financeiro · Documentos da obra · Relatórios ·
Configurações por obra
...
- **Backend:** Supabase (Auth, PostgreSQL, Storage, RLS versionado em
  `supabase/migrations/`)
...
_Documento consolidado a partir da auditoria de junho/2026. Substituí todos os
arquivos em `docs/v1/` e `docs/lgpd/`. Revisar após cada sprint significativo._
