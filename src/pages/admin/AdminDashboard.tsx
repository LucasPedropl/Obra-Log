...
export default function AdminDashboard() {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminService.createCompanyAdmin(selectedCompany.id, adminEmail);
      ...
      <aside className="w-full md:w-80 bg-white border-r border-slate-200 flex flex-col h-screen sticky top-0">
        <div className="p-6 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-900 leading-tight">Super-Admin</h1>
              <p className="text-xs text-slate-500 font-medium">Gestão de Tenants</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-b border-slate-200 bg-slate-50/50">
          <form onSubmit={handleCreateCompany} className="flex flex-col gap-3">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Nova Empresa</h2>
            {companyError && <div className="p-2 bg-red-50 text-red-700 rounded-lg border border-red-100 text-xs">{companyError}</div>}
            <div className="flex gap-2">
              ...
              />
              <button disabled={loading} className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white p-2 rounded-lg transition-colors flex items-center justify-center shadow-sm">
                {loading ? <Loader2 className="animate-spin w-5 h-5" /> : <Plus size={20} />}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {loadingData ? (
              ...
              <p className="text-sm text-slate-500 text-center py-8">Nenhuma empresa encontrada.</p>
            ) : (
              filteredCompanies.map(company => (
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${selectedCompany?.id === company.id ? 'bg-indigo-200 text-indigo-700' : 'bg-slate-200 text-slate-500'}`}>
                      <Building2 size={16} />
                    </div>
                    <div className="truncate">
                      <div className="font-semibold text-sm truncate">{company.name}</div>
                      <div className="text-[10px] opacity-60 font-mono mt-0.5">{company.id.split('-')[0]}</div>
                    </div>
                  </div>
                  <ChevronRight size={16} className={selectedCompany?.id === company.id ? 'text-indigo-400' : 'text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity'} />
                </button>
              ))
            )}
          </div>
        ...
        {selectedCompany ? (
          <div className="max-w-4xl mx-auto p-8 md:p-12">
            <div className="mb-10">
              ...
              <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight">{selectedCompany.name}</h2>
              <p className="text-slate-500 mt-2 text-lg">Gerencie os acessos administrativos e configurações desta empresa.</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 flex flex-col">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">Novo Acesso Admin</h3>
                    <p className="text-xs text-slate-500">Gere credenciais para o cliente</p>
                  </div>
                </div>
                ...
                {result ? (
                  <div className="flex-1 flex flex-col justify-center">
                    <div className="p-6 bg-emerald-50 border border-emerald-200 rounded-xl text-center">
                      ...
                      <h3 className="text-emerald-800 font-bold text-lg mb-2">Acesso Gerado!</h3>
                      <p className="text-emerald-700 mb-6 text-sm">Envie estas credenciais para o cliente.</p>
                      
                      <div className="bg-white p-4 rounded-lg border border-emerald-100 font-mono text-sm text-left space-y-2">
                        <div className="flex flex-col">
                          <span className="text-xs text-slate-400 uppercase tracking-wider">E-mail</span>
                          <span className="font-medium text-slate-800">{result.email}</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-xs text-slate-400 uppercase tracking-wider">Senha Temporária</span>
                          <span className="font-medium text-slate-800">{result.tempPass}</span>
                        </div>
                      </div>
                  ...
                  <form onSubmit={handleCreateUser} className="flex-1 flex flex-col">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-3 leading-relaxed">
                        O usuário receberá uma senha temporária e será solicitado a definir seu nome e uma nova senha no primeiro acesso.
                      </p>
                    </div>
                    
                    <button disabled={loading} className="mt-8 w-full bg-slate-900 hover:bg-indigo-600 disabled:bg-slate-300 text-white font-medium py-3 px-6 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-sm">
                      {loading ? <Loader2 className="animate-spin w-5 h-5" /> : <Plus size={18} />} Gerar Credenciais
                    </button>
                  </form>
                )}
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                <div className="p-6 border-b border-slate-200 flex items-center gap-3 bg-slate-50/50">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">Usuários Ativos</h3>
                    <p className="text-xs text-slate-500">Administradores da empresa</p>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-2">
                  {loadingUsers ? (
                    ...
                    <div className="p-12 text-center flex flex-col items-center">
                      ...
                      <p className="text-sm text-slate-400 mt-1">Gere o primeiro acesso ao lado.</p>
                    </div>
                  ) : (
          ...
          <div className="h-full flex flex-col items-center justify-center p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Nenhuma empresa selecionada</h2>
            <p className="text-slate-500 max-w-md">
              Selecione uma empresa no menu lateral para gerenciar seus acessos ou crie uma nova empresa para começar.
            </p>
          </div>
        )}
      </main>
